...
export async function analyzeManifest(manifest: Manifest, opts: AnalyzeOptions = {}): Promise<Report> {
  ...
  // Lockstep attribution is gated on the frameworks actually present among the direct deps —
  // membership alone would misattribute shared packages (`react` → "Expo" in a Next.js app).
